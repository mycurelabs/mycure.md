...
export default async function ClinicProfilePage({ params }: PageParams) {
  const profile = await getProfileBySlug(params.slug)
  if (!profile) notFound()
  // by-slug does not filter type — send people to the doctor route.
  if (profile.type !== "organization") redirect(`/d/${profile.slug}`)
